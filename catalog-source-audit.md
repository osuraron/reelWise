...
Poster-credits verification: the same source-backed Director/Created by and Main cast content now sits directly below the poster and Reelwise selection caption, rather than in the title-and-synopsis column. Desktop validation showed the compact credits block held within the poster column; at 375px, it remained visibly attached to the poster above the title detail block, with no duplicated credits in the text column.

Fit-score hierarchy verification: the “Reelwise selection / year” poster caption has been removed. The focused recommendation now presents the algorithmic result as a labelled “Reelwise fit” score immediately above the title, while retaining the separate TMDB community rating in the top control row. With a temporary seeded profile, the validated live title showed a Reelwise fit of 86% above the title on both desktop and 375px mobile, with no caption below the poster.

Fit-marker verification: the inline Reelwise fit label has been replaced with a percentage-only 86% marker, enlarged and anchored to the top-right of the main recommendation card. The distinct TMDB community rating remains in the separate top control row. DOM checks confirmed the marker stays within the main card at desktop and mobile breakpoints, and the prior inline label is absent.

Clean-card verification: the percentage-only personalized marker was removed at the user’s request. Desktop and 375px mobile live recommendation views now return to the uncluttered title-and-metadata composition, while the underlying score remains available in the Why this panel and the separate TMDB community rating remains in the top control row.

Near-black dark-mode verification: shared app, onboarding, navigation, poster, and recommendation-card surfaces now use a neutral near-black hierarchy rather than olive charcoal. Desktop live recommendation inspection confirmed an `rgb(8, 8, 8)` app background, a subtle dark-card layer, legible warm-ivory title text, and retained signal-red accents. Desktop and 375px onboarding views retained clear borders, readable secondary text, and visible controls.
